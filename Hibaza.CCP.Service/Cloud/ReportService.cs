...
namespace Hibaza.CCP.Service
{
    public class ReportService : IReportService
    {
        ...
        {
            Paging page = new Paging { Limit = 100, Previous = (filter.init > 99999999999 ? filter.init/ 1000 : filter.init).ToString(), Next = (filter.finish> 99999999999 ? filter.finish / 1000 : filter.finish).ToString() };
            IEnumerable<ReportAgentDataLine> list = await _reportRepository.GetAgentChartData(business_id, page);
        ...
        {
            Paging page = new Paging { Limit = filter.limit, Previous = Core.Helpers.CommonHelper.UnixTimestampToDateTime(filter.init > 99999999999 ? filter.init / 1000 : filter.init).ToString(), Next = filter.finish > 99999999999 ? (filter.finish / 1000).ToString() : filter.finish.ToString() };
        ...
        {
            Paging page = new Paging { Limit = filter.limit, Previous = (filter.init > 99999999999 ? filter.init / 1000 : filter.init).ToString(), Next = (filter.finish > 99999999999 ? filter.finish/ 1000 : filter.finish).ToString() };
        ...
        {   
            _reportRepository.UpsertReportAll();
        }
