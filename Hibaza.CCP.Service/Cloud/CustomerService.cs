...
namespace Hibaza.CCP.Service
{
    public class CustomerService : ICustomerService
    {
        ...
        public Task<Domain.Models.CustomerContactInfoModel> GetCustomerId(string business_id, string id)
        {
            return _customerRepository.GetCustomerId(business_id, id);
        }
        public async Task<int> AutoAssignToAvailableAgents(string business_id, Paging page)
        {
            //11111
            if (page == null)
                page = new Paging { Limit = Convert.ToInt32(_appSettings.Value.LimitAssign) };
            ...
            counters = UnAssignFromOverLoadAgents(business_id, channels, agents, counters, page.Limit, new Paging { Limit = 1000 });
            ...
            var customers = (await GetPendingNonReplyCustomers(business_id, new Paging { Limit = 1000 })).Where(c => channels.ContainsKey(c.channel_id));

            int count = 0;
            List<string> agentKeys = agents.Where(a => a.Value.status == "online").OrderBy(b => counters.ContainsKey(b.Key) ?
            counters[b.Key].unread : 0).Select(a => a.Key).ToList();
            ...
            {
                count++;
                if (counters.ContainsKey(agentKeys[i]))
                {
                    while (i < agentKeys.Count &&
                        counters[agentKeys[i]].unread >= page.Limit) i++;
        ...
        {
            int count = 0;
            List<string> agentKeys = agents.Where(a => a.Value.status == "online" || (a.Value.status == "busy" &&
            DateTime.Now.Subtract(a.Value.last_acted_time.Value).TotalMinutes <= 2)).Select(a => a.Key).ToList();
            var customers = GetActiveNonReplyCustomers(business_id, page).Result;
            foreach (var t in customers)
            {
                //if (!string.IsNullOrWhiteSpace(t.agent_id))
                //{
                //t.agent_id = (agentKeys.Contains(t.agent_id) && (agents[t.agent_id].status == "online" || t.agent_id != t.assigned_by)) ? t.agent_id : null;
                t.agent_id = (agentKeys.Contains(t.agent_id) && (agents[t.agent_id].status == "online" || t.agent_id == t.assigned_by)) ?
                    t.agent_id : null;
        ...
        public Dictionary<string, Counter> UnAssignFromOverLoadAgents(string business_id, Dictionary<string, Channel> channels, Dictionary<string, Agent> agents, Dictionary<string, Counter> counters, int limit, Paging page)
        {
            List<string> agentKeys = agents.Where(a => a.Value.status == "online" || (a.Value.status == "busy" &&
            DateTime.Now.Subtract(a.Value.last_acted_time.Value).TotalMinutes <= 2)).Select(a => a.Key).ToList();
            var customers = GetActiveUnreadCustomers(business_id, page).Result;
            foreach (var t in customers.Where(c => c.assigned_by is null).OrderByDescending(c => c.timestamp))
            {
                var key = "";
                //t.agent_id = (agentKeys.Contains(t.agent_id) && (agents[t.agent_id].status == "online" || t.agent_id != t.assigned_by)) ? t.agent_id : null;
                //if (!string.IsNullOrWhiteSpace(t.agent_id))
                //{
                key = t.agent_id;
                t.agent_id = (agentKeys.Contains(t.agent_id)
                    && counters.ContainsKey(t.agent_id) && counters[t.agent_id].unread <= limit
                    && (agents[t.agent_id].status == "online" || t.agent_id == t.assigned_by)) ?
                    t.agent_id : null;
                t.status = channels.ContainsKey(t.channel_id) ? (string.IsNullOrWhiteSpace(t.agent_id) ? "pending" : "active") : null;
                if (t.status != "active")
                {
                    if (!string.IsNullOrWhiteSpace(key) && counters.ContainsKey(key))
                    {
                        counters[key].unread--;
                        counters[key].count--;
                    }
        ...
        {
            return await _customerRepository.GetPendingNonReplyCustomers(business_id, page);
        }
        ...
        {
            return await _customerRepository.GetActiveNonReplyCustomers(business_id, page);
        }
        ...
        public bool AssignToAgent(string business_id, string customer_id, string agent_id, string assigned_by)
        {
            ...
            //AutoAssignToAvailableAgents(business_id, page);
        ...
        {
            return await _customerRepository.GetCustomerActiveThread(business_id);
        }

        public async Task<List<Customer>> GetCustomersActiveThreadLikeThread(string business_id, string thread_id, Paging page)
        {
            return await _customerRepository.GetCustomersActiveThreadLikeThread( business_id, thread_id, page);
        }
        public async Task<List<Customer>> GetCustomersAppId(string business_id, string appId)
        {
            return await _customerRepository.GetCustomersAppId(business_id, appId);
        }
        public async Task<List<Customer>> GetCustomerFromPhone(string business_id, string phone)
        {
            return await _customerRepository.GetCustomerFromPhone(business_id, phone);
        }

        public async Task<List<Customer>> GetCustomerFromPhone(string phone)
        {
            return await _customerRepository.GetCustomerFromPhone(phone);
        }

        public async Task<bool> UpdateRealName(string business_id, string customer_id, string real_name)
        {
          return await  _customerRepository.UpdateRealName(business_id,  customer_id,  real_name);
        }
    }
}
